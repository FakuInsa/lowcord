...
powershell -NoProfile -Command "try { $rc = ''; if (Test-Path '%~dp0room_code.txt') { $rc = (Get-Content '%~dp0room_code.txt' -Raw).Trim() }; if ($rc) { Invoke-RestMethod -Uri ('https://api.keyval.org/set/lowcord_' + $rc + '/offline') -TimeoutSec 2 -ErrorAction SilentlyContinue } } catch {}" >nul 2>&1
